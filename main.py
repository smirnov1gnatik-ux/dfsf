...
async def compute_snapshot(user_id:int):
    ...
    entries = [
        ...
        ("ZRO", prof["fzro"], pzro, pct(pzro, bzro)),  # пустышка отображается как ZRO
    ]
    total = 0.0
...
def schedule_user_job(user_id:int, hour:int, minute:int, bot: Bot, playwright):
    # Удалим старую задачу с тем же id
    ...
    async def job():
        ...
        if err:
            await bot.send_message(user_id, err)
            return
        path = await render_wallet_screenshot(playwright, snap["rows"], snap["total"])
        await bot.send_photo(user_id, FSInputFile(path), caption=f"Плановый снимок {datetime.utcnow().strftime('%Y-%m-%d %H:%M UTC')}")

    scheduler.add_job(job, trigger, id=job_id)

# ---------- Бот ----------
dp = Dispatcher()
PLAYWRIGHT = None  # глобальный инстанс
...
async def cmd_setup(m: Message):
    ...
    # Настроим расписание, если задано
    if sched:
